// Place at: src/FormsTemplateHome.jsx
// The "Forms" template's own home page - same rich list Home.jsx (the old
// Lab-only "Your Forms" page) already had: search, grid/list toggle,
// pinning, Draft/Live/Paused/Archived states with per-status actions, a
// Recycle Bin. Scoped to just this template's own forms (settings.
// templateSlug === 'forms') instead of every unaffiliated form on the
// account, and reached via the literal /templates/forms/locations route in
...
function PinIcon({ size = 14 }) {
  return (
    <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke="#9a9a9a" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{ flexShrink: 0 }}>
...
function FormsTemplateHome() {
  ...
  usePageTitle(template?.name || 'Forms')
  usePageBack('/', 'All Businesses')

  useEffect(() => {
    async function loadTemplate() {
      const { data, error: templateErr } = await supabase.from('templates').select('*').eq('slug', TEMPLATE_SLUG).single()
      if (templateErr || !data) {
        setTemplateError('This template could not be found.')
        setLoading(false)
        return
      }
      setTemplate(data)
  ...
  useEffect(() => {
    ...
    async function loadForms() {
      const { data, error } = await supabase
        ...
        .eq('settings->>templateSlug', TEMPLATE_SLUG)
  ...
  // actual button. NavBar isn't a descendant of this page, so it can't call
  ...
  // A brand new form under this template needs a name up front (same as
  // every other template's "+ Add Location") - createLocationForm requires
  // one, and it's what the form starts out named until its own builder
  // renames it.
  ...
  return (
    <div className="page">
      ...
      <div className="toolbar-row" style={{ justifyContent: 'space-between', marginBottom: '1.5rem' }}>
        <h1 style={{ margin: 0 }}>{template?.name || 'Forms'}</h1>
        <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center' }}>
          ...
          <button onClick={openAddModal} disabled={!template}>+ New Form</button>
        </div>
      </div>
      ...
      {!loading && forms.length === 0 && (
        <div className="card" style={{ padding: '2.5rem', textAlign: 'center', color: 'var(--color-muted)' }}>
          ...
          <button style={{ marginTop: '0.5rem' }} onClick={openAddModal} disabled={!template}>Create your first form</button>
        </div>
      )}
            ...
            <>
              {pinnedForms.length > 0 && (
                <div style={{ marginBottom: '2rem' }}>
                  ...
                  {viewMode === 'list' ? (
                    <ListView pageForms={pinnedForms} {...sharedProps} openMenuId={openMenuId} setOpenMenuId={setOpenMenuId} menuRef={menuRef} />
                  ) : (
                    <GridView pageForms={pinnedForms} {...sharedProps} openMenuId={openMenuId} setOpenMenuId={setOpenMenuId} menuRef={menuRef} />
                  )}
                </div>
              )}
              ...
              {unpinnedForms.length > 0 && (
                <div>
                  ...
                  {viewMode === 'list' ? (
                    <ListView pageForms={pageForms} {...sharedProps} openMenuId={openMenuId} setOpenMenuId={setOpenMenuId} menuRef={menuRef} />
                  ) : (
                    <GridView pageForms={pageForms} {...sharedProps} openMenuId={openMenuId} setOpenMenuId={setOpenMenuId} menuRef={menuRef} />
                  )}
      ...
      {showAddModal && (
        <Modal size="sm" onClose={() => setShowAddModal(false)} title="Name this form">
          <form onSubmit={confirmAddForm}>
            <input
...
export default FormsTemplateHome
