...
// Memoized, and its own KPI computation (buildKpis, moved out to
// report/analysis/buildKpis.js) is further wrapped in useMemo below - that
// computation alone used to account for most of the ~40+ map/filter/sort/
// reduce calls the perf audit flagged. None of that has anything to do with
// `selectedMore`/`metricSearch`/`valueFormat`, so typing in the metric
// search box or toggling a checkbox no longer re-runs it - and memo() means
// an unrelated Report re-render (Options menu, PDF progress,
...
const KPIGrid = memo(function KPIGrid({ form, submissions, previousSubmissions = [], totalResponses, moreMenuOpen, setMoreMenuOpen }) {
  ...
  // The computation itself lives in report/analysis/buildKpis.js - shared
  // with the Print/PDF Designer's own KPI sidebar (PrintWorkspace.jsx) so
  // the two can never list different metrics for the same form.
  const { primaryKpis, moreKpis } = useMemo(
    () => buildKpis({ form, submissions, previousSubmissions, totalResponses }),
