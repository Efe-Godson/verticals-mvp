...
async function finalizeRoom(supabase: any, roomId: string) {
  ...
  // instead of one UPDATE per player - see the matching migration for why.
  if (players?.length) {
    const { error: rankError } = await supabase.rpc('finalize_quiz_room_ranks', {
      p_room_id: roomId,
      p_player_ids: players.map((p: any) => p.id),
      p_ranks: players.map((_: any, i: number) => i + 1),
    })
