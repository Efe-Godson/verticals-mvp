-- Extends workflow_transfers (20260916120000) to also support transferring
-- a single location instead of always the whole workflow. scope='workflow'
-- keeps every existing row/behavior exactly as it was; scope='location'
-- moves just one location's form (plus any bundle secondaries that name it
-- as their primaryFormId, e.g. a payroll location's Salary Events form -
-- see createBundleTemplateForms in src/locations.js) to the recipient,
-- leaving the rest of the business where it is.
alter table public.workflow_transfers
  add column scope text not null default 'workflow' check (scope in ('workflow','location')),
  add column form_id uuid references public.forms(id);

-- The old single index conflated the two scopes: a pending workflow
-- transfer for a template would block a location transfer under that same
-- template (same owner_id+template_slug), and two different locations in
-- the same business would block each other too. Split by scope instead -
-- workflow transfers still key off (owner, template), location transfers
-- key off (owner, form).
drop index public.workflow_transfers_one_pending;
create unique index workflow_transfers_one_pending_workflow on public.workflow_transfers(owner_id, template_slug) where status in ('sending','pending') and scope = 'workflow';
create unique index workflow_transfers_one_pending_location on public.workflow_transfers(owner_id, form_id) where status in ('sending','pending') and scope = 'location';

-- CREATE OR REPLACE only replaces a function with the exact same parameter
-- TYPE list - two new trailing params (p_scope, p_form_id) make this a
-- different signature, so without an explicit drop the old 6-arg version
-- would stick around as a second overload. That second overload would then
-- make every rpc() call that omits p_scope/p_form_id (cancel/review/accept/
-- decline - see manage-workflow-transfer/index.ts) ambiguous between the two,
-- since both have defaults for everything past p_hash - breaking those
-- actions for every transfer, old and new.
drop function if exists public.manage_workflow_transfer(uuid,text,text,text,uuid,text);
...
begin
  ...
  -- The workflow-wide collaborator_shares clause only applies when the
  -- whole workflow is moving - a location transfer must never touch shares
  -- that grant access to sibling locations staying with the old owner.
...
$$;
revoke all on function public.manage_workflow_transfer(uuid,text,text,text,uuid,text,text,uuid) from public, anon, authenticated;
grant execute on function public.manage_workflow_transfer(uuid,text,text,text,uuid,text,text,uuid) to service_role;
